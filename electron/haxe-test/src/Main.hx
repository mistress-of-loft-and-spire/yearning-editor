...
class Main extends hxd.App {
    ...
    override function init() {
		...
		var mat2 = h3d.mat.Material.create(tex);
		...
		cast(s3d.lightSystem,h3d.scene.fwd.LightSystem).ambientLight.set(0.5, 0.4, 0.9);

        Engine.getCurrent().backgroundColor = 0x2520f9;

		// disable shadows
		obj1.material.shadows = false;
		obj2.material.shadows = false;

        obj1.material.mainPass.wireframe = true;
	...
	override function update( dt : Float ) {
		...
		obj1.setRotationAxis(-0.1, 1, Math.cos(time), time + Math.PI / 2);
	}
